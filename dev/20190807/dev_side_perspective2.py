...
section_plane = OCC.gp.gp_Pln(
    OCC.gp.gp_Pnt(xmax+xmin, section_width, 0.0),
    OCC.gp.gp_Dir(0, 1, 0)
)
section_face = OCC.BRepBuilderAPI.BRepBuilderAPI_MakeFace(section_plane, zmin, zmax, xmin, xmax).Face()
...
while exp.More():
    ...
    for face in tp.faces():
        ...
        for edge in list(Topo(face).edges()):
            ...
            if handle_adaptor.GetType() == GeomAbs_Line:
            ...
            elif handle_adaptor.GetType() == GeomAbs_BSplineCurve :
                
                v = list(Topo(e).vertices())
                v1 = (BRep_Tool.Pnt(v[0]).X(), BRep_Tool.Pnt(v[0]).Z())
                v2 = (BRep_Tool.Pnt(v[-1]).X(), BRep_Tool.Pnt(v[-1]).Z())
                
                bspline = handle_adaptor.BSpline().GetObject()
                ...
                poles = [(bspline.Pole(index).X(), bspline.Pole(index).Y()) for index in range(1, bspline.NbPoles()+1)]
                ...
                if 4.88 in x and np.any(y>=40) and np.any(y<=60):
                    plt.plot(y, x, color="blue", alpha=0.2)
                    ifcopenshell.geom.utils.display_shape(e, clr=BLUE)
                else:
                    plt.plot(y, x, color="red", alpha=0.2)
                    ifcopenshell.geom.utils.display_shape(e, clr=RED)
